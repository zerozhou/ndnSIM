...
// In the below, we will accomplish network layer and application layer
   ...
   /* 
    ndnHelper.AddRoute(wifiNodes.Get(0), "/ustc/segments", grid.GetNode(0,1), 1); //这里是不需要的，因为application 会做这个事情
    ndnHelper.AddRoute(grid.GetNode(0,0), "/ustc/segments", grid.GetNode(0,1), 1);
    ndnHelper.AddRoute(grid.GetNode(0,1), "/ustc/segments", grid.GetNode(1,1), 1);
    ndnHelper.AddRoute(grid.GetNode(1,1), "/ustc/segments", grid.GetNode(1,2), 1);
    ndnHelper.AddRoute(grid.GetNode(1,2), "/ustc/segments", grid.GetNode(2,2), 1);
    //NodeContainer wifiApNode1=grid.GetNode(0,0);
 */
   ...
   // ndnHelper.InstallAll();
    ...
    ndnGlobalRoutingHelper.Install(grid.GetNode(0,0));
    ndnGlobalRoutingHelper.Install(grid.GetNode(0,1));
    ndnGlobalRoutingHelper.Install(grid.GetNode(1,1));
    ndnGlobalRoutingHelper.Install(grid.GetNode(2,1));
    ndnGlobalRoutingHelper.Install(grid.GetNode(2,2));
...
*/
    std::cout << "test\n";
    ndn::AppHelper appHelper("MyFirstApp");
    appHelper.Install(wifiNodes);
    appHelper.Install(grid.GetNode(2,2));
    ndnGlobalRoutingHelper.AddOrigins ( "/ustc/segments", grid.GetNode(2,2) );
